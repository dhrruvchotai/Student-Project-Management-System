...
const upcomingMeetings = [
  { id: 1, title: "Weekly Progress Review", date: "Oct 24, 2:00 PM", status: "Scheduled", location: "Lab 3" }, // 
  { id: 2, title: "Phase 2 Evaluation", date: "Nov 01, 10:00 AM", status: "Pending", location: "Conf Room" },
];
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 relative overflow-hidden">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 relative z-10">
          <div>
            ...
            <p className="text-zinc-400 text-sm">Overview of your academic project progress</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard label="Attendance" value="85%" icon={Users} color="text-emerald-500" delay={0.1} />
          <StatCard label="Tasks Pending" value="3" icon={Clock} color="text-orange-500" delay={0.2} />
          <StatCard label="Meetings Done" value="12" icon={CheckCircle2} color="text-blue-500" delay={0.3} />
          <StatCard label="Documents" value="5" icon={FileText} color="text-purple-500" delay={0.4} />
        </div>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <h2 className="text-xl font-bold text-white mb-2">{studentProject.title}</h2>
              ...
              <div className="mb-6">
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="text-white font-medium">{studentProject.progress}%</span>
                </div>
              ...
              <div className="flex items-center justify-between border-t border-white/5 pt-4">
                <div className="flex -space-x-2">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="h-8 w-8 rounded-full bg-zinc-800 border-2 border-zinc-900 flex items-center justify-center text-xs font-medium text-white">
                      {i === 1 ? 'L' : 'M'}
                    </div>
                  ))}
            ...
            >
              ...
              <div className="space-y-4">
                {upcomingMeetings.map((meeting) => (
                  <div key={meeting.id} className="flex items-center gap-4 p-4 rounded-xl bg-black/40 border border-white/5 hover:border-indigo-500/30 transition-colors group">
                    <div className="h-12 w-12 rounded-xl bg-zinc-800 flex flex-col items-center justify-center border border-white/5">
                       <span className="text-xs text-zinc-500 font-medium uppercase">{meeting.date.split(',')[0].split(' ')[0]}</span>
                       <span className="text-lg font-bold text-white">{meeting.date.split(',')[0].split(' ')[1]}</span>
                    </div>
                    <div className="flex-1">
                      <h4 className="text-white font-medium mb-1 group-hover:text-indigo-400 transition-colors">{meeting.title}</h4>
                      <div className="flex items-center gap-3 text-xs text-zinc-500">
                        <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {meeting.date.split(',')[1]}</span>
                        <span className="flex items-center gap-1"><Users className="h-3 w-3" /> {meeting.location}</span>
                      </div>
                    </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="p-3 bg-zinc-800 hover:bg-zinc-700 rounded-xl flex flex-col items-center justify-center gap-2 transition-all border border-white/5 hover:border-white/10">
                  ...
                  <span className="text-xs font-medium text-gray-300">Upload Doc</span>
                </button>
                <button className="p-3 bg-zinc-800 hover:bg-zinc-700 rounded-xl flex flex-col items-center justify-center gap-2 transition-all border border-white/5 hover:border-white/10">
                  <MessageSquare className="h-5 w-5 text-emerald-400" />
                  <span className="text-xs font-medium text-gray-300">Contact Guide</span>
                </button>
                 <button className="p-3 bg-zinc-800 hover:bg-zinc-700 rounded-xl flex flex-col items-center justify-center gap-2 transition-all border border-white/5 hover:border-white/10">
                  <Calendar className="h-5 w-5 text-orange-400" />
                  <span className="text-xs font-medium text-gray-300">Schedule</span>
                </button>
                <button className="p-3 bg-zinc-800 hover:bg-zinc-700 rounded-xl flex flex-col items-center justify-center gap-2 transition-all border border-white/5 hover:border-white/10">
